...
/**A QMatrix is a mutable, quadruply linked matrix ADT. Each row is circularly linked 
 * and each column is circularly linked. The row and column of sentinel nodes that result 
 * are also circularly linked, and share the main sentinel node. 
 * 
...
public class QMatrix<E> implements Iterable<E>{
  ...
  /**
   * newNode simply returns a new QMatrixNode<E> that is appropriate for this matrix 
   * (override this method to use a different type of node)
   * @param matrix the matrix this node is contained in
   * @param item the item this node stores
   * @param up the node above this one
  ...
  /**
   * QMatrix() constructs an empty QMatrix
   * Keep in mind that when adding a new row or column to an empty QMatrix, 
   * in addition to creating a new row or column sentinel, one must also
   * create for each new node a new column or row sentinel, respectively.
   **/
  public QMatrix(){
  ...
  public static void main(String[] args){
    ...
    System.out.println("Rows: " + foo.numRows() + " Columns: " + foo.numColumns());
  ...
  public QColumn<E> insertColumnFront(Iterable<E> newColumnItems){
    ...
    this.head.right.left = this.newNode(null, this.head.row, null, null, null, null, this.head, this.head.right);
    ...
    while(newColumnIterator.hasNext()){
      if(this.n == 0){
        current.left.down = this.newNode(null, null, current.left.column, null, current.left, this.head, null, null);
        current.left.down.row = new QRow<E>(this, current.left.down);
        this.m++;
      }
      current.down = this.newNode(this, current.left.down.row, current.column, newColumnIterator.next(), current, this.head.right, current.left.down, current.right.down);
  ...
  public QColumn<E> insertColumnBack(Iterable<E> newColumnItems){
    QMatrixNode<E> current;
    this.head.left.right = this.newNode(null, this.head.row, null, null, null, null, this.head.left, this.head);
    ...
    while(newColumnIterator.hasNext()){
      if(this.n == 0){
        current.right.down = this.newNode(null, null, current.right.column, null, current.right, this.head, null, null);
        current.right.down.row = new QRow<E>(this, current.right.down);
        this.m++;
      }
      current.down = this.newNode(this, current.right.down.row, current.column, newColumnIterator.next(), current, this.head.left, current.left.down, current.right.down);
      current.right.down.left = current.down;
      current.left.down.right = current.down;
      if(!newColumnIterator.hasNext()){
  ...
  public QRow<E> insertRowTop(Iterable<E> newRowItems){
    QMatrixNode<E> current;
    this.head.down.up = this.newNode(null, null, this.head.column, null, this.head, this.head.down, null, null);
    ...
    while(newRowIterator.hasNext()){
      if(this.m == 0){
        current.up.right = this.newNode(null, current.up.row, null, null, null, null, current.up, this.head);
        current.up.right.column = new QColumn<E>(this, current.up.right);
        this.n++;
      }
      current.right = this.newNode(this, current.row, current.up.right.column, newRowIterator.next(), current.up.right, current.down.right, current, this.head.down);
  ...
  public QRow<E> insertRowBottom(Iterable<E> newRowItems){
    QMatrixNode<E> current;
    this.head.up.down = this.newNode(null, null, this.head.column, null, this.head.up, this.head, null, null);
    ...
    while(newRowIterator.hasNext()){
      if(this.m == 0){
        current.down.right = this.newNode(null, current.down.row, null, null, null, null, current.down, this.head);
        current.down.right.column = new QColumn<E>(this, current.down.right);
        this.n++;
      }
      current.right = this.newNode(this, current.row, current.down.right.column, newRowIterator.next(), current.up.right, current.down.right, current, this.head.up);
      current.down.right.up = current.right;
      current.up.right.down = current.right;
      if(!newRowIterator.hasNext()){
